            ...
            if (tempItems.Count > 0)
            {
                ...
                foreach (GameObject item in tempItems)
                {
                    ...
                    while (t.childCount > 0)
                    {
                        ...
                        child.SetParent(t.parent, false);
                        child.position = pos;
        ...
        public override void OnToolGUI(EditorWindow window)
        {
            ...
            if (phase == 0)
            {
                ...
                if (EditorGUI.EndChangeCheck())
                {
                    ...
                    sourceBounds = new Bounds(sourceGameObjects[0].transform.position, Vector3.zero);
                    sourceSize = SelectionBoundsManager.bounds.size;

                    for (int i = 1; i < sourceGameObjects.Length; i++)
                    {
                        sourceBounds.Encapsulate(sourceGameObjects[i].transform.position);
                    }
