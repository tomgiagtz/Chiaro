...
namespace InfinityCode.uContext.MainLayout
{
    public class BreadcrumbsLayout : MainLayoutItem
    {
        ...
        private void DrawNeighborsButton()
        {
            ...
            {
                GameObject go = item;
                menu.AddItem(new GUIContent(go.name), Selection.activeGameObject == go, () =>
                {
        ...
        private void GetChildren(Transform t, GenericMenu menu, string prefix)
        {
            for (int i = 0; i < t.childCount; i++)
            {
                ...
                if (ct.childCount > 0)
                {
                    menu.AddItem(new GUIContent(title + "/" + "Select"), false, () =>
                    {
                        Selection.activeGameObject = go;
                        SceneViewManager.OnNextGUI += uContextMenu.ShowInLastPosition;
                    });
        ...
        public override void OnGUI()
        {
            ...
            {
                foreach (GameObject target in targets) target.SetActive(enabled);
            }
