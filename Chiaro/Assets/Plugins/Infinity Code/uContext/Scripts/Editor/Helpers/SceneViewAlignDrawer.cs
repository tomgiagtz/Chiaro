...
namespace InfinityCode.uContext.FloatToolbar
{
    public static class SceneViewAlignDrawer
    {
        ...
        private static void OnSceneGUI(SceneView sceneView)
        {
            ...
            float height = sceneView.position.height - 21;
            float left = QuickAccess.visible ? QuickAccess.width : 0;

            if (PrefabStageUtility.GetCurrentPrefabStage() != null) height -= 25;
            ...
            GUI.DrawTexture(new Rect(width - 5, 0, 5, 20), texture, ScaleMode.StretchToFill);
            GUI.DrawTexture(new Rect(width - 20, 0, 15, 5), texture, ScaleMode.StretchToFill);

            // Bottom right
            texture = align == ToolbarAlign.bottomRight ? activeTexture : passiveTexture;
            GUI.DrawTexture(new Rect(width - 5, height - 20, 5, 20), texture, ScaleMode.StretchToFill);
            GUI.DrawTexture(new Rect(width - 20, height - 5, 15, 5), texture, ScaleMode.StretchToFill);
        ...
        public static ToolbarAlign Update(Rect sceneRect, Rect wndRect, out Vector2 offset)
        {
            ...
            if (PrefabStageUtility.GetCurrentPrefabStage() != null) sceneRect.height -= 25;
            if (QuickAccess.visible) sceneRect.xMin += QuickAccess.width;
