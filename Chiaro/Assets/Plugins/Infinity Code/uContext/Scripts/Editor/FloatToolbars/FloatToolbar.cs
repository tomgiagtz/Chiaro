    ...
    public abstract class FloatToolbar
    {
        ...
        public void OnSceneViewGUI(SceneView sceneView)
        {
            ...
            try
            {
                ...
                PrepareToolbar(sceneView.position);
        ...
        private void ProcessHeaderRelease(Rect sceneRect, Rect headerRect)
        {
            ...
            if (e.button == 0)
            {
                ...
                if ((e.mousePosition - pressMousePosition).sqrMagnitude < 10)
                {
                    OnHeaderClick();
                }
            }
        ...
        protected virtual void UpdateRect(Rect sceneRect)
        {
            ...
            if (QuickAccess.visible)
            {
                rect.x += QuickAccess.width;
                sceneSize.x -= QuickAccess.width;
            }

            if (hasVerticalCenterAlign) rect.x += (sceneSize.x - size.x) / 2;
            else if (hasRightAlign) rect.x += sceneSize.x - size.x;

            if (PrefabStageUtility.GetCurrentPrefabStage() != null) sceneHeight -= 25;
