...
namespace InfinityCode.uContextPro.Windows
{
    ...
    public static class ViewGalleryCameras
    {
        ...
        {
            if (GUILayout.Button("Cameras", EditorStyles.toolbarButton, GUILayout.ExpandWidth(false)))
            {
                GenericMenu menu = new GenericMenu();

                menu.AddItem(new GUIContent("Remove All Temporary Cameras"), false, RemoveAllTemporaryCameras, gallery);

                foreach (ViewGallery.CameraStateItem cam in gallery.cameras.Where(c => c.camera.GetComponentInParent<TemporaryContainer>() != null))
                {
                    menu.AddItem(new GUIContent("Remove " + cam.camera.gameObject.name), false, RemoveTemporaryCamera, cam);
                }
        ...
        private static void OnDrawCameras(ViewGallery gallery, float rowHeight, float maxLabelWidth, ref int offsetY, ref int row)
        {
            ...
            for (int i = 0; i < cameras.Length; i++)
            {
                ...
                {
                    SceneViewHelper.AlignViewToCamera(cameraState.camera);
